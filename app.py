...
def start_location_flow(event, state_prefix):
    user_id = event.source.user_id
    database.set_user_state(user_id, f'{state_prefix}_waiting_for_pref')
    area_data = get_area_data()
    if not area_data:
        reply_to_line(event.reply_token, [{"type": "text", "text": "地域情報の取得に失敗しました。"}])
        return
    pref_names = [pref.get('title') for pref in area_data.findall('.//pref')]
    quick_reply = create_quick_reply_dict(sorted(list(set(pref_names))))
    reply_to_line(event.reply_token, [{"type": "text", "text": "都道府県を選択してください。", "quickReply": quick_reply.to_dict()}])
...
def handle_message(event):
    ...
    flow_type = None
    if user_state and user_state.startswith('register'): flow_type = 'register'
    elif user_state and user_state.startswith('lookup'): flow_type = 'lookup'
    
    if user_state == f'{flow_type}_waiting_for_pref':
        ...
        if selected_pref:
            ...
            reply_to_line(event.reply_token, [{"type": "text", "text": "最後に都市名を選択してください。", "quickReply": quick_reply.to_dict()}])
        else:
            reply_to_line(event.reply_token, [{"type": "text", "text": "ボタンから正しい都道府県名を選択してください。"}])
    
    elif user_state and user_state.startswith(f'{flow_type}_waiting_for_city:'):
